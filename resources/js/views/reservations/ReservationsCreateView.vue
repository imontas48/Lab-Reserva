...
const handleEquipmentSelection = () => {
  console.log('📝 Equipo seleccionado:', selectedEquipmentPreview.value?.name || selectedEquipmentPreview.value?.identifier);
    ...
    <div v-if="!selectedLab" class="rounded-lg bg-white p-6 shadow-sm border border-gray-200">
      ...
      <div v-if="loadingLabs" class="flex items-center justify-center py-12">
        <div class="text-center">
          ...
          <p class="mt-4 text-sm text-gray-600">Cargando laboratorios...</p>
        </div>
      </div>

      <!-- Error -->
      <div v-else-if="errorLabs" class="rounded-lg border border-red-200 bg-red-50 p-4">
        <div class="flex">
          <svg class="h-5 w-5 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
          </svg>
          <div class="ml-3">
            <h3 class="text-sm font-medium text-red-800">Error al cargar laboratorios</h3>
            <p class="mt-1 text-sm text-red-700">{{ errorLabs }}</p>
          </div>
        </div>
      </div>
      ...
      <div v-else-if="activeLabs.length > 0" class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <button
          ...
          :key="lab.id"
          @click="selectLab(lab)"
          class="group relative flex flex-col rounded-lg border border-gray-200 bg-white p-6 shadow-sm transition-all hover:border-blue-500 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
        >
    ...
    <!-- ========================================================================= -->
    <div v-else-if="selectedLab && !selectedEquipment" class="rounded-lg bg-white p-6 shadow-sm border border-gray-200">
      ...
      <div class="mb-6 rounded-lg bg-blue-50 p-4 border border-blue-200">
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              <p class="text-sm font-medium text-blue-900">Laboratorio seleccionado:</p>
              <h3 class="text-lg font-semibold text-blue-900">{{ selectedLab.name }}</h3>
              <p class="text-sm text-blue-700">{{ selectedLab.location }}</p>
            </div>
          </div>
          <button
            @click="clearLabSelection"
            class="inline-flex items-center rounded-lg border border-blue-300 bg-white px-4 py-2 text-sm font-medium text-blue-700 shadow-sm hover:bg-blue-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
          >
      ...
      <div v-if="loadingEquipment" class="flex items-center justify-center py-12">
        <div class="text-center">
          ...
          <p class="mt-4 text-sm text-gray-600">Cargando equipos del laboratorio...</p>
        </div>
      </div>

      <!-- Error equipos -->
      <div v-else-if="errorEquipment" class="rounded-lg border border-red-200 bg-red-50 p-4">
        <div class="flex">
          <svg class="h-5 w-5 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
          </svg>
          <div class="ml-3">
            <h3 class="text-sm font-medium text-red-800">Error al cargar equipos</h3>
            <p class="mt-1 text-sm text-red-700">{{ errorEquipment }}</p>
          </div>
        </div>
      </div>
      ...
      <div v-else-if="labEquipment.length > 0">
        ...
        <div v-if="selectedEquipmentId" class="mt-4 rounded-lg bg-gray-50 p-4 border border-gray-200">
          <h4 class="text-sm font-medium text-gray-900 mb-3">Equipo seleccionado:</h4>
          <div class="flex items-center justify-between">
            <div class="flex items-center">
    ...
    <div v-else class="space-y-6">
      <!-- Breadcrumb completo -->
      <div class="rounded-lg bg-gradient-to-r from-blue-50 to-green-50 p-4 border border-blue-200">
        <div class="flex items-center justify-between flex-wrap gap-4">
          ...
          <div class="flex items-center flex-wrap gap-4">
            ...
            <div class="flex items-center">
              ...
              <div class="ml-3">
                <p class="text-xs font-medium text-blue-900">Laboratorio:</p>
                <p class="text-sm font-semibold text-blue-900">{{ selectedLab.name }}</p>
              </div>
            </div>
            ...
            <div class="flex items-center">
              ...
              <div class="ml-3">
                <p class="text-xs font-medium text-green-900">Equipo:</p>
